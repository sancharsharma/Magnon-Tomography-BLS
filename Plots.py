...
# We expect a folder inside Figures and .hf file with the same name <category>
category = 'Cat_WithSq'
...
def _contour_plot_(vals, args, fig=None, ax=None):
	...
	axis_ticks = [-near_max,0,near_max]
	ax.set_xticks(axis_ticks)
	ax.set_yticks(axis_ticks)

	if (vals.min() > -0.05*vals.max()): # If the minimum is too close to 0, we don't need to put it on the colobar
...
args = np.array(data['parameters/wigner_args'])
tar_wig = np.array(data['target/wigner'])
recons_wigs = [np.array(data[name + '/wigner']) for name in grp_names]
data.close()
